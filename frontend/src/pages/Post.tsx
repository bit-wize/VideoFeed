...
import { CREATE_COMMENT } from "../graphql/mutations/createCommentMutation"
import { GET_COMMENTS_BY_POST } from "../graphql/queries/GetCommentsByPost"
import { useMutation, useQuery } from "@apollo/client"
import { DELETE_COMMENT } from "../graphql/mutations/deleteCommentMutation"
import { GetCommentsByPostQuery } from "../gql/graphql"
import { usePostStore } from "../stores/postStore"
import { useUserStore } from "../stores/userStore"
import { LIKE_POST } from "../graphql/mutations/likeMutation"
import { UNLIKE_POST } from "../graphql/mutations/unlikeMutation"
...
function Post() {
  ...
  const [deleteComment] = useMutation(DELETE_COMMENT, {
    update(cache, { data: { deleteComment } }) {
      ...
      const existingComments = cache.readQuery<GetCommentsByPostQuery>({
        query: GET_COMMENTS_BY_POST,
        variables: { postId: Number(id) },
      })

      const newComments = existingComments?.getCommentsByPost.filter(
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center px-8 mt-8">
          <div className="pb-4 text-center flex items-center">
            ...
            >
              <AiOutlineLike
                size="25"
                className={"text-[#838383] focus:text-[#a855f7] hover:text-[#a855f7] " + (isLiked ? "text-[#838383]" : "text-[#a855f7]")}
              />

            </button>
            <span className="text-xs pl-2 pr-4 text-[#838383] font-semibold">
              {dataPost?.getPostById?.likes.length}
            </span>
          </div>
          <div className="pb-4 text-center flex items-center">
            <div className="rounded-full bg-gray-200 p-2 cursor-pointer">
              <FaRegCommentAlt size="25" color="text-[#a855f7]" />
            </div>
        ...
        >
          <div className="pt-2" />
          {data?.getCommentsByPost.length === 0 && (
